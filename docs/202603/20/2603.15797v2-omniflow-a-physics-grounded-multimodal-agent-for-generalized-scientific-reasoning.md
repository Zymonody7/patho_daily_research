...
motivation: 现有大模型在处理受物理定律约束的连续时空动态数据时，往往因缺乏物理常识而产生不符合物理规律的预测结果。
method: 提出一种神经符号架构，利用语义-符号对齐将高维流场转化为拓扑描述符，并配合物理引导的思维链进行动态约束注入。
result: 在微观湍流、纳维-斯托克斯方程及全球天气预报等跨领域任务中，该方法在零样本和少样本场景下的表现均显著优于传统黑盒深度学习模型。
conclusion: 该研究证明了通过将物理定律作为约束注入多模态智能体，可以实现从单纯的数据拟合向具备物理一致性和可解释性的科学推理转变。
---
