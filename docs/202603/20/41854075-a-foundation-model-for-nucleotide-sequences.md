...
## 速览
**TLDR**：现有的基因组基础模型大多仅关注核苷酸序列，忽略了丰富的注释信息，限制了跨物种和生物场景的泛化能力。OmniNA 通过在 9170 万条序列及其关联的 1.97 亿个注释词汇上进行自监督生成式预训练，首次实现了序列与语义注释的联合学习。该模型在序列检测、物种分类等 23 项基准测试中达到顶尖水平，并能有效预测突变对 DNA/RNA 加工的影响，为基因组学研究提供了强大的多模态基础工具。 \
**Motivation**：现有的基因组模型往往只学习核苷酸序列，忽略了与其紧密相关的生物学注释信息，导致模型对复杂生物背景的理解不足。 \
**Method**：提出了 OmniNA 模型，通过在大规模核苷酸序列及其对应的文本注释上进行联合自监督预训练，使模型能够同时掌握序列语法和生物语义。 \
**Result**：该模型在包括序列识别和物种分类在内的 23 个下游任务中表现优异，并能准确揭示基因突变对生物过程的影响。 \
**Conclusion**：OmniNA 证明了整合注释信息能显著提升核苷酸基础模型的表征能力，为基因组和转录组学研究提供了一个通用的开源工具。
