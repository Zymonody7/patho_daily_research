...
# 默认抓取范围保持兼容，但优先允许用户在 config.yaml 中收紧为更聚焦的分类。
DEFAULT_CATEGORIES_TO_FETCH = [
...
def resolve_categories_to_fetch(config: dict | None) -> list[str]:
    paper_setting = (config or {}).get("arxiv_paper_setting") or {}
    raw_categories = paper_setting.get("categories")
    if not isinstance(raw_categories, list):
        return list(DEFAULT_CATEGORIES_TO_FETCH)
    ...
    return categories or list(DEFAULT_CATEGORIES_TO_FETCH)
...
def fetch_all_domains_metadata_robust(
    ...
    config = load_config()
    categories_to_fetch = resolve_categories_to_fetch(config)
    ...
    # 2. 遍历分类进行抓取
    log(f"🧭 [Global Ingest] Categories: {', '.join(categories_to_fetch)}")
