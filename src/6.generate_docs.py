...
def generate_deep_summary(md_file_path: str, txt_file_path: str, max_retries: int = 3) -> str | None:
    ...
    system_prompt = (
        "你是一名资深学术论文分析助手。请使用清晰、直白、少术语堆砌的中文，"
        "以 Markdown 形式输出结构化总结。默认读者是做 AI 方向的研究者，"
        "他们未必熟悉具体生物学背景，所以你需要先讲清问题，再讲方法。"
    )
    user_prompt = (
        "请基于下面提供的论文内容，生成一段详细的中文总结，要求按照如下要点依次展开：\n"
        "1. 这篇论文到底在解决什么问题，为什么值得看。\n"
        "2. 先用 3 到 5 句话做“白话版概述”，让非该细分方向读者也能快速懂。\n"
        "3. 方法部分：核心思想、模型结构、训练或推理流程、关键设计取舍。\n"
        "4. 实验部分：数据、任务、baseline、评价指标、主要结果。\n"
        "5. 资源与算力：如果文中提到，请总结训练资源；若没有，请明确写“文中未充分披露”。\n"
        "6. 这篇论文真正可信的贡献是什么，哪些结论证据最强。\n"
        "7. 局限与风险：包括实验不足、外推风险、数据偏差、真实应用障碍。\n"
        "8. 对 AI for Bioinformatics 的启发：一句“适合关注的人群”和一句“后续可跟进的问题”。\n\n"
        "请用分层标题和项目符号（Markdown 格式）组织上述内容。避免空话，避免把摘要换个说法重复一遍，"
        "遇到生物学术语时用一句短解释补足背景。\n"
...
def generate_glance_overview(title: str, abstract: str, max_retries: int = 3) -> str | None:
    ...
    system_prompt = (
        "你是论文速览助手。请用易懂、准确、面向 AI 研究者的中文总结论文，"
        "避免空泛评价，避免术语堆砌。"
    )
    payload = {"title": title, "abstract": abstract}
    user_text = json.dumps(payload, ensure_ascii=False)
    user_prompt = (
        ...
        "要求：\n"
        "- tldr：100字左右的完整概述，先交代问题，再说明方法，再点出结果和价值\n"
        "- motivation/method/result/conclusion：每个字段一句话概括，尽量让非该领域读者也能读懂\n"
        "- 不要使用“本文提出了一种有效方法”这类空话，要说清提出了什么、用在什么任务上\n"
        "Output must be strict JSON only, no markdown, no fences, no extra text."
    )
...
def _list_meta_index_paths(docs_dir: str) -> List[str]:
    paths: List[str] = []
    for root, _dirs, files in os.walk(docs_dir):
        if "papers.meta.json" in files:
            paths.append(os.path.join(root, "papers.meta.json"))
...
def build_knowledge_base(docs_dir: str, max_days: int = 60, max_items: int = 500) -> tuple[str, str]:
    payloads: List[Dict[str, Any]] = []
    for path in _list_meta_index_paths(docs_dir):
        try:
            with open(path, "r", encoding="utf-8") as f:
    ...
    for payload in payloads:
        ...
        day_dt = _parse_iso_datetime(day) or _parse_iso_datetime(str(payload.get("generated_at") or ""))
        if day_dt and (now - day_dt).days > max_days:
            continue
        for paper in payload.get("papers") or []:
            if not isinstance(paper, dict):
                continue
            paper_id = str(paper.get("paper_id") or "").strip()
            title = str(paper.get("title_en") or "").strip()
            if not paper_id or not title:
                continue
            key = f"{paper_id}|{title}"
            if key in seen:
                continue
            seen.add(key)
            item = dict(paper)
    ...
    json_path = os.path.join(docs_dir, "knowledge-base.json")
    md_path = os.path.join(docs_dir, "knowledge-base.md")
    ...
    lines.append("# 知识库")
    lines.append("")
    lines.append(f"- 最近沉淀条目数：{len(entries)}")
    lines.append("- 用途：把每日抓取、每日解读的结果沉淀成可检索的阅读索引。")
    lines.append("")
    lines.append("## 最近论文")
    lines.append("")
    for item in entries[:200]:
        ...
        if RANGE_DATE_RE.match(date_value):
            link = f"#/{date_value}/{paper_id}"
        elif re.fullmatch(r"\d{4}-\d{2}-\d{2}", date_value):
            compact = date_value.replace("-", "")
            link = f"#/{compact[:6]}/{compact[6:]}/{paper_id}"
        else:
            link = f"#/{paper_id}"
...
def main() -> None:
    ...
    log_substep("6.8", "更新知识库索引", "START")
    try:
        kb_md, kb_json = build_knowledge_base(docs_dir)
        log(f"[OK] knowledge base updated: {kb_md}, {kb_json}")
    except Exception as e:
        log(f"[WARN] 更新知识库索引失败：{e}")
    log_substep("6.8", "更新知识库索引", "END")
